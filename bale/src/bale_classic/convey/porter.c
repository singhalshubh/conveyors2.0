...
porter_grab_buffers(porter_t* self)
{
  ...
  // madvise(self->send_buffers, size << self->abundance, MADV_NOHUGEPAGE);
  // madvise(self->recv_buffers, size << shift, MADV_NOHUGEPAGE);
...
void
porter_free(porter_t* self, int i)
{
  if (self == NULL)
    return;
  ...
  #ifdef NETWORK_DEBUG
    fprintf(stderr, "Porter %d, Network: %ld cycles\n", i, self->network_time);
  #endif
  //if(self->sync_count != 0)
  //fprintf(stderr, "Async calls: %ld, pe: %ld\n", self->async_calls/self->sync_count, shmem_my_pe());
