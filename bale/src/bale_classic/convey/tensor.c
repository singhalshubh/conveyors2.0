...
// #define TIME_DEBUG
// #define INTER_PORTER_TIME
// #define WASTED_PULL
#define CRAY
// #define USEFUL_TIME_DEBUG_PUSH
...
tensor_push(convey_t* self, const void* item, int64_t pe)
{
  ...
  #ifdef USEFUL_TIME_DEBUG_PUSH
    uint64_t tot = _rdtsc() - start_1;
    if(ok) {tensor->useful_push_time += tot;}
    tensor->push_time += tot;
  #endif
  #ifdef TIME_DEBUG
      tensor->push_time += _rdtsc() - start;
  #endif
...
tensor_pull(convey_t* self, void* item, int64_t* from)
{
  ...
  if (source == NULL) {
    #ifdef WASTED_PULL
      tensor->wasted_pull_time += _rdtsc() - start;
    #endif
    return convey_FAIL;
  }
  memcpy(item, source, self->item_size);
  #ifdef TIME_DEBUG
    tensor->pull_time += _rdtsc() - start;
  #endif
  return convey_OK;
}
...
tensor_advance(convey_t* self, bool done)
{
  ...
  for (int k = tensor->n_complete; k < order; k++) {
    ...
    for (int loop = 0; loop < 5; loop++) {
      ...
      #ifdef INTER_PORTER_TIME
        tensor->inter_porter_time += _rdtsc() - start_1;
      #endif
  ...
  #ifdef TIME_DEBUG
    tensor->advance_time += _rdtsc() - start;
  #endif
...
{
  //fprintf(stderr, "MATRIX_HOP: %ld\n", MATRIX_REMOTE_HOP);
  tensor_t* tensor = (tensor_t*) self;
  #ifdef TIME_DEBUG
    tensor->push_time = 0;
    tensor->pull_time = 0;
    tensor->advance_time = 0;
  #endif

  #ifdef USEFUL_TIME_DEBUG_PUSH
    tensor->push_time = 0;
    tensor->useful_push_time = 0;
  #endif

  #ifdef INTER_PORTER_TIME
    tensor->inter_porter_time = 0;
  #endif

  #ifdef WASTED_PULL
    tensor->wasted_pull_time = 0;
  #endif
...
tensor_free(convey_t* self)
{
  tensor_t* tensor = (tensor_t*) self;
  #ifdef TIME_DEBUG
    fprintf(stderr, "Push-time: %ld cycles\n", tensor->push_time);
    fprintf(stderr, "Pull-time: %ld cycles\n", tensor->pull_time);
    fprintf(stderr, "Advance-time: %ld cycles\n", tensor->advance_time);
    fprintf(stderr, "Total-time: %ld cycles\n", tensor->push_time + tensor->advance_time + tensor->pull_time);
  #endif

  #ifdef INTER_PORTER_TIME
    fprintf(stderr, "Inter-Porter Time %ld cycles\n", tensor->inter_porter_time);
  #endif

  #ifdef USEFUL_TIME_DEBUG_PUSH
    fprintf(stderr, "Push-time: %ld cycles\n", tensor->push_time);
    fprintf(stderr, "Useful-Push-time: %ld cycles\n", tensor->useful_push_time);
  #endif

  #ifdef WASTED_PULL
    fprintf(stderr, "Wasted Pull-time: %ld cycles\n", tensor->wasted_pull_time);
  #endif

  if (!mpp_comm_is_equal(MPP_COMM_CURR, tensor->comm))
    return convey_error_TEAM;
  for (int i = tensor->order - 1; i >= 0; i--) {
    porter_free(tensor->porters[i], i);
  }
  free(tensor->aligned_item);
  free(self);
  return convey_OK;
